import React from "react";

const EventsBlock = () => {
    return (
        <>
            <div className="member story story--style2 padding-bottom overflow-hidden">
                <div className="container">
                    <div className="section__header style-2 text-center wow fadeInUp" data-wow-duration="1.5s">
                        <h2>Liste des derniers événements ajoutés</h2>
                        <p>Listen and learn from our community members and find out tips and tricks to meet your love. Join us and be part of a bigger family.</p>
                    </div>
                    <div className="section__wrapper">
                        <div className="member__info mb-4">
                            <div className="member__info--left">
                                <div className="member__info--filter">
                                    <div className="default-btn" /* data-bs-toggle="modal" data-bs-target="#exampleModal" */ >
                                        <span><i className="fa-solid fa-plus-circle"></i> Evenement</span>
                                    </div>
                                </div>
                                <div className="member__info--count">
                                    <div className="default-btn"><span>All Members</span></div>
                                    <p>20365587</p>
                                </div>
                            </div>
                            <div className="member__info--right">
                                <div className="member__info--customselect right w-100">
                                    <div className="default-btn"><span>Order By:</span></div>
                                    <div className="banner__inputlist">
                                        <select>
                                            <option value="0">Last Active</option>
                                            <option value="1">Oldest</option>
                                            <option value="2">Popular</option>
                                            <option value="3">Most Active</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="row g-4 justify-content-center row-cols-lg-4 row-cols-sm-2 row-cols-1">
                            <div className="col">
                                <div className="story__item style2 story--theme-color">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/01.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">2 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Be young, love life</h4></a>
                                            <p>You become what you believe. –Oprah Winfrey</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/01.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/02.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/03.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/04.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/05.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color2">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/02.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">4 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Learn how to love</h4></a>
                                            <p>Fall seven times and stand up eight. –Japanese Proverb</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/06.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/07.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/08.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/09.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/10.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color3">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/03.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">8 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Love, health and...</h4></a>
                                            <p>Definiteness of purpose is the starting point of all</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/11.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/12.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/13.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/14.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color4">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/04.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">12 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Ollya Dating Theme...</h4></a>
                                            <p> will most likely be using this theme really nice dating theme for</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/01.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/06.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/03.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/07.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/02.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color5">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/05.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">18 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Skip relation routine</h4></a>
                                            <p>Twenty years from now you will be more disappointed</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/11.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/03.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/09.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/07.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color6">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/06.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">21 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Gay Community</h4></a>
                                            <p>It’s your place in the world; it’s your life. </p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/12.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/10.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/01.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/08.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color7">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/07.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">24 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Only Buddies</h4></a>
                                            <p>You miss 100% of the shots you don’t take.</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/02.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/09.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/04.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/08.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color8">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/08.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">30 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Love Stories</h4></a>
                                            <p>You become what you believe. –Oprah Winfrey</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/01.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/07.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/14.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/02.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/05.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color9">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/09.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">12 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Be young, love life</h4></a>
                                            <p>You become what you believe. –Oprah Winfrey</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/03.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/01.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/02.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/04.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/10.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color10">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/10.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">12 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Continually bandwidth rather.</h4></a>
                                            <p>Fall seven times and stand up eight. –Japanese Proverb</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/11.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/05.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/04.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/07.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color11">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/11.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">19 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Conveniently simplify exceptional.</h4></a>
                                            <p>Fall seven times and stand up eight. –Japanese Proverb</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/12.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/09.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/06.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/08.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/05.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col">
                                <div className="story__item style2 story--theme-color12">
                                    <div className="story__inner">
                                        <div className="story__thumb position-relative">
                                            <a href="group-single.html"><img src="assets/images/group/12.jpg"
                                                                             alt="dating thumb"/></a>
                                            <span
                                                className="member__activity member__activity--ofline">22 days ago</span>
                                        </div>
                                        <div className="story__content px-0 pb-0">
                                            <a href="group-single.html"><h4>Seamlessly architect scalable.</h4></a>
                                            <p>Fall seven times and stand up eight. –Japanese Proverb</p>
                                            <div
                                                className="story__content--author justify-content-between border-top pt-3">
                                                <div className="story__content--content">
                                                    <p><i className="fa-solid fa-earth-africa"></i> Public Group</p>
                                                </div>
                                                <ul className="img-stack">
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/13.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span className="tooltip-info">Klaus Klug</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/15.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Mary C. Shen</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/10.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Lucas Vogler</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/09.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Manuela Moench</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="#">
                                                            <img src="assets/images/group/icon/06.png" alt="member-img"/>
                                                                <div className="time-tooltip">
                                                                    <div className="time-tooltip-holder">
                                                                        <span
                                                                            className="tooltip-info">Matthias Propst</span>
                                                                    </div>
                                                                </div>
                                                        </a>
                                                    </li>
                                                    <li className="bg-theme">
                                                        <a href="#">
                                                            12+
                                                            <div className="time-tooltip">
                                                                <div className="time-tooltip-holder">
                                                                    <span className="tooltip-info">More</span>
                                                                </div>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="member__pagination mt-4">
                            <div className="member__pagination--left">
                                <p>Viewing 1 - 20 of 12,345 Members</p>
                            </div>
                            <div className="member__pagination--right">
                                <ul className="default-pagination">
                                    <li>
                                        <a href="#"><i className="fas fa-chevron-left"></i></a>
                                    </li>
                                    <li>
                                        <a href="#">01</a>
                                    </li>
                                    <li>
                                        <a href="#" className="active">02</a>
                                    </li>
                                    <li>
                                        <a href="#">03</a>
                                    </li>
                                    <li>
                                        <a href="#"><i className="fas fa-chevron-right"></i></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}

export default EventsBlock;